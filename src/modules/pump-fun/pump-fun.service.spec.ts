...
jest.mock('../../utils/transaction.utils', () => ({
  ...
  isBondingCurveComplete: jest.fn().mockReturnValue(false),
...
describe('PumpFunService', () => {
      ...
      providers: [
        {
          ...
          useFactory: () => {
            return new PumpFunService('https://mock-rpc-url.com');
          },
        },
      ],
    }).compile();
  ...
  describe('isTokenPumpFun', () => {
    it('should return true for a valid Pump.fun token', async () => {
      const result = await service.isTokenPumpFun('ValidTokenMint');
      expect(result).toBe(true);
    });
  });

  describe('isBondingCurveComplete', () => {
    it('should return false for an incomplete bonding curve', async () => {
      const result = await service.isBondingCurveComplete('TokenMint');
      expect(result).toBe(false);
    });

    it('should return true for a complete bonding curve', async () => {
      // Mock the fetchBondingCurveAccount to return a complete bonding curve
      const { fetchBondingCurveAccount } = require('../../utils/account.utils');
      fetchBondingCurveAccount.mockResolvedValueOnce({
        virtualTokenReserves: BigInt(100),
        virtualSolReserves: BigInt(200),
        realTokenReserves: BigInt(50),
        realSolReserves: BigInt(100),
        tokenTotalSupply: BigInt(1000),
        complete: true // Set to complete
      });
